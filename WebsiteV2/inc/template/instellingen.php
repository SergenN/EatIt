...
// Een functie die ervoor zorgt dat de Klantgegevens worden aangepast in de database
		function update_data($name, $email, $con, $newvalue){
			// De update query
...
// Een functie die ervoor zorgt dat de Medewerkersgegevens worden aangepast in de database
		function update_data2($name, $email, $con, $newvalue){
			// De update query
...
if($_SESSION['soortgebruiker'] == "klant"){
	...
	if(isset($_POST['submit_settings'])){
		...
		if ($_POST['email'] != '') {
			$email = $_POST['email'];
			update_data('KL_Mail', $gegevens['KL_Mail'], $con, $email);
		}

		if ($_POST['voornaam'] != '') {
			$voornaam = $_POST['voornaam'];
			update_data('KL_Voornaam', $gegevens['KL_Mail'], $con, $voornaam);
		}

		if ($_POST['achternaam'] != '') {
			$achternaam = $_POST['achternaam'];
			update_data('KL_Achternaam', $gegevens['KL_Mail'], $con, $achternaam);
		}

		if ($_POST['telefoonnummer'] != '') {
			$telefoonnummer = $_POST['telefoonnummer'];
			update_data('KL_Telefoonnummer', $gegevens['KL_Mail'], $con, $telefoonnummer);
		}

		if ($_POST['plaats'] != '') {
			$plaats = $_POST['plaats'];
			update_data('KL_Plaats', $gegevens['KL_Mail'], $con, $plaats);
		}

		if ($_POST['adres'] != '') {
			$adres = $_POST['adres'];
			update_data('KL_Adres', $gegevens['KL_Mail'], $con, $adres);
		}

		if ($_POST['postcode'] != '') {
			$postcode = $_POST['postcode'];
			update_data('KL_Postcode', $gegevens['KL_Mail'], $con, $postcode);
		}
		...
		// Alle nieuwe gegevens van de ingelogde gebruiker opslaan in de sessie variabel
			// $query = "SELECT * FROM Klant WHERE email = '" . $gegevens['KL_Mail'] . "' ";
			// $result = mysqli_query($con, $query);
...
if($_SESSION['soortgebruiker'] == "medewerker"){
	...
	if(isset($_POST['submit_settings'])){
		...
		if ($_POST['email'] != '') {
			$email = $_POST['email'];
			update_data2('MED_Mail', $gegevens['MED_Mail'], $con, $email);
		}

		if ($_POST['voornaam'] != '') {
			$voornaam = $_POST['voornaam'];
			update_data2('MED_Voornaam', $gegevens['MED_Mail'], $con, $voornaam);
		}

		if ($_POST['achternaam'] != '') {
			$achternaam = $_POST['achternaam'];
			update_data2('MED_Achternaam', $gegevens['MED_Mail'], $con, $achternaam);
		}

		if ($_POST['telefoonnummer'] != '') {
			$telefoonnummer = $_POST['telefoonnummer'];
			update_data2('MED_Telefoonnummer', $gegevens['MED_Mail'], $con, $telefoonnummer);
		}

		if ($_POST['plaats'] != '') {
			$plaats = $_POST['plaats'];
			update_data2('MED_Plaats', $gegevens['MED_Mail'], $con, $plaats);
		}

		if ($_POST['adres'] != '') {
			$adres = $_POST['adres'];
			update_data2('MED_Adres', $gegevens['MED_Mail'], $con, $adres);
		}

		if ($_POST['postcode'] != '') {
			$postcode = $_POST['postcode'];
			update_data2('MED_Postcode', $gegevens['MED_Mail'], $con, $postcode);
		}
